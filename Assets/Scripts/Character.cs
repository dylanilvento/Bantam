...
public class Character : MonoBehaviour
{
    ...
    public Image[] uiBullets = new Image[4],
        reloadBarFill = new Image[5],
        uiHealth = new Image[4];
    public Sprite bulletFill,
        bulletOutline,
        heartFill,
        heartOutline;
    int ammo = 3,
    ...
    void Shoot()
    {
        if (!ducking)
        {
            GameObject spawnBullet = Instantiate(
                bullet,
                new Vector2(
                    transform.position.x + CheckDirectionForBulletSpawn(),
                    transform.position.y + 0.03f
                ),
                Quaternion.identity
            );
            if (transform.localScale.x < 0)
                spawnBullet.transform.localScale = new Vector2(
                    spawnBullet.transform.localScale.x * -1,
                    spawnBullet.transform.localScale.y
                );
        }
        else
        {
    ...
    void OnCollisionEnter2D(Collision2D other)
    {
        if (
            ...
            || other.gameObject.name.Contains("Enemy Base")
